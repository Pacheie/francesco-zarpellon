...
form.addEventListener('submit', e => {
  ...
  setTimeout(() => {
    showFeedback('success', 'Messaggio inviato con successo! Ti risponderò al più presto.');
    form.reset();
    btn.disabled = false;
...
/* ── 8. CV DOWNLOAD placeholder ────────────────────────────────── */
document.getElementById('btn-cv').addEventListener('click', e => {
  e.preventDefault();
  // Replace with the real CV file path, e.g.: window.open('cv-francesco-zarpellon.pdf')
  alert('CV non ancora disponibile.\nSostituisci questo alert con il link al tuo file CV.');
});
